    ...
    let res = match FromPrimitive::from_u32(comp) {
        ...
        Some(RGD::Sgt) => {
            if (a as i64) > (b as i64) {
                ...
                b.overflowing_sub(a).0.saturating_add(1)
            }
        }
        Some(RGD::Sle) => {
            if (a as i64) <= (b as i64) {
        ...
        Some(RGD::Slt) => {
            if (a as i64) < (b as i64) {
                ...
                a.overflowing_sub(b).0.saturating_add(1)
            }
        }
        Some(RGD::Uge) => {
            if a >= b {
...
pub fn single_distance(
    min_input: &MutInput,
    constraints: &Vec<Rc<RefCell<Cons>>>,
    scratch_args: &mut Vec<u64>,
    distances: &mut Vec<u64>,
    dimension_index: usize,
    cmap: &HashMap<usize, Vec<usize>>,
) {
...
pub fn cal_gradient(fut: &mut Fut) {
    ...
    for (i, g) in grad.iter_mut().enumerate() {
        let (sign, _islinear, val, solved) = partial_derivative(
            orig_input,
            scratch_args,
            distances,
            orig_distances,
