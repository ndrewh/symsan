...
impl<'a> JITEngine<'a> {
    ...
    fn codegen<'b>(
        ...
        fn_val: FunctionValue<'b>,
        value_cache: &mut HashMap<u32, IntValue<'b>>,
    ) -> Option<IntValue<'b>> {
        ...
        let result = match FromPrimitive::from_u32(request.get_kind()) {
            ...
            Some(RGD::Constant) => {
                ...
                let idx = unsafe {
                    builder.build_gep(
                        input_args,
                        &[i32_type.const_int(start as u64 + RET_OFFSET, false)],
                        "argidx",
                    )
                };
                let mut ret = builder.build_load(idx, "argidx").into_int_value();
                ret = builder.build_int_truncate(
                    ret,
                    self.context.custom_width_int_type(request.get_bits()),
                    "truncate",
                );
                Some(ret)
            }
            Some(RGD::Read) => {
                ...
                for k in 1..length {
                    let idx = unsafe {
                        builder.build_gep(
                            input_args,
                            &[i32_type.const_int((start + k) as u64 + RET_OFFSET, false)],
                            "argidx",
                        )
                    };
            ...
            Some(RGD::Extract) => {
                ...
                if c1.is_some() {
                    ...
                    Some(builder.build_int_truncate(
                        builder.build_right_shift(c1.unwrap(), shift_idx, false, "lshr"),
                        self.context.custom_width_int_type(request.get_bits()),
                        "truncate",
                    ))
                } else {
            ...
            Some(RGD::UDiv) => {
                ...
                if c1.is_some() && c2.is_some() {
                    ...
                    let cond =
                        builder.build_int_compare(IntPredicate::EQ, c2.unwrap(), va0, "icmpeq");
                    let divisor = builder
                        .build_select(cond, va1, c2.unwrap(), "select")
                        .into_int_value();
            ...
            Some(RGD::SDiv) => {
                ...
                if c1.is_some() && c2.is_some() {
                    ...
                    let cond =
                        builder.build_int_compare(IntPredicate::EQ, c2.unwrap(), va0, "icmpeq");
                    let cond1 =
                        builder.build_int_compare(IntPredicate::EQ, c1.unwrap(), vam1, "icmpeq");
                    let cond2 =
                        builder.build_int_compare(IntPredicate::EQ, c2.unwrap(), vam2, "icmpeq");
                    let cond3 = builder.build_and(cond1, cond2, "land");
                    let divisor = builder
                        .build_select(cond, va1, c2.unwrap(), "select")
                        .into_int_value();
                    let divisor1 = builder
                        .build_select(cond3, minus2, divisor, "select")
                        .into_int_value();
            ...
            Some(RGD::Equal) | Some(RGD::Distinct) | Some(RGD::Ult) | Some(RGD::Ule)
            | Some(RGD::Ugt) | Some(RGD::Uge) => {
                ...
                if c1.is_some() && c2.is_some() {
                    ...
                    let input_args = fn_val.get_nth_param(0).unwrap().into_pointer_value();
                    let idx0 = unsafe {
                        builder.build_gep(input_args, &[i32_type.const_int(0, false)], "argidx")
                    };
                    let idx1 = unsafe {
                        builder.build_gep(input_args, &[i32_type.const_int(1, false)], "argidx")
                    };
            ...
            Some(RGD::Slt) | Some(RGD::Sle) | Some(RGD::Sgt) | Some(RGD::Sge) => {
                let left = &request.get_children()[0];
                let right = &request.get_children()[1];
                let type_after = self.context.custom_width_int_type(64);
                let c1 = self.codegen(builder, &left, local_map, fn_val, value_cache);
    ...
    pub fn add_function(&self, request: &AstNode, local_map: &HashMap<u32, u32>) -> Option<usize> {
        ...
        let module = self.context.create_module(&module_id);
        let i64_type = self.context.i64_type();
        let i64_pointer_type = i64_type.ptr_type(AddressSpace::default());
        ...
        if let Some(body) = self.codegen(&builder, request, local_map, fn_val, &mut value_cache) {
            ...
            */
            ...
            //remove module from engine to save space
...
mod tests {
    ...
    fn test_add() {
        ...
        unsafe {
            ...
            let add: JitFunction<unsafe extern "C" fn(i32, i32) -> i32> =
    ...
    fn test_jitengine_pointer_load() {
        ...
        let mut x: [u64; 2] = [10, 12];
        unsafe {
            println!("result is {}", fun.call(x.as_mut_ptr()));
        }
    }
    ...
    fn test_jitengine_add() {
        ...
        unsafe {
            println!("result is {}", fun.call(1, 2));
        }
    }
}
