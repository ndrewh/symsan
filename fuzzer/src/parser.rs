...
fn eq_cur(current: &AstNode, other: &AstNode) -> bool {
    ...
    if current.get_hash() != other.get_hash() {
        return false;
    }
    if current.get_kind() != other.get_kind() {
        return false;
    }
    if current.get_bits() != other.get_bits() {
        return false;
    }
    let children_size = current.get_children().len();
    if current.get_children().len() != other.get_children().len() {
...
impl<'a> SearchTaskBuilder<'a> {
    ...
    pub fn add_dependency(&mut self, task: &SearchTask, inputs: &HashSet<u32>, v0: u32) {
        ...
        let mut onecons = OneLabelCons {
            members: all_branch_cons,
        };
        ...
        if is_empty {
            self.branch_deps[v0 as usize] = Some(BranchDep {
                cons_set: Vec::new(),
            });
        }
    ...
    pub fn append_meta(
        ...
        for amap in constraint.get_meta().get_map() {
            cons.borrow_mut()
                .local_map
                .insert(amap.get_k(), amap.get_v());
        }
        for aarg in constraint.get_meta().get_args() {
            cons.borrow_mut()
                .input_args
                .push((aarg.get_isinput(), aarg.get_v()));
        }
        for ainput in constraint.get_meta().get_inputs() {
            cons.borrow_mut()
                .inputs
                .insert(ainput.get_offset(), ainput.get_iv() as u8);
        }
        for ashape in constraint.get_meta().get_shape() {
            cons.borrow_mut()
                .shape
                .insert(ashape.get_offset(), ashape.get_start());
        }
        let mut comp = constraint.get_node().get_kind();
        if (flip) {
            comp = get_flipped_op(comp);
        }
        if comp == 0 {
            return false;
        }
